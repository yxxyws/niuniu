...
public static class NGUIText
{
	...
	public static float GetGlyphWidth(int ch, int prev)
	{
		...
		{
			if (NGUIText.dynamicFont != null && NGUIText.dynamicFont.GetCharacterInfo((char)ch, out NGUIText.mTempChar, NGUIText.finalSize, NGUIText.fontStyle))
			{
	...
	public static void Align(BetterList<Vector3> verts, int indexOffset, float printedWidth, int elements = 4)
	{
		...
		if (alignment != NGUIText.Alignment.Right)
		{
			...
			else
			{
				...
				{
					Vector3[] expr_F2_cp_0 = verts.buffer;
					int expr_F2_cp_1 = j;
					expr_F2_cp_0[expr_F2_cp_1].x = expr_F2_cp_0[expr_F2_cp_1].x + num11;
				}
			}
		}
		else
		{
			...
			{
				Vector3[] expr_48_cp_0 = verts.buffer;
				int expr_48_cp_1 = k;
				expr_48_cp_0[expr_48_cp_1].x = expr_48_cp_0[expr_48_cp_1].x + num14;
			}
		}
	}
		...
		while (i < length)
		{
			...
			else
			{
				if (NGUIText.encoding)
				{
					...
					{
						if (NGUIText.ParseSymbol(text, ref i, NGUIText.mColors, NGUIText.premultiply, ref num6, ref flag4, ref flag5, ref flag6, ref flag7, ref flag8))
						{
							if (flag8)
							{
	...
	public static void Print(string text, BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		for (int i = 0; i < length; i++)
		{
			...
			else
			{
				...
				{
					if (NGUIText.encoding && NGUIText.ParseSymbol(text, ref i, NGUIText.mColors, NGUIText.premultiply, ref num8, ref flag2, ref flag3, ref flag4, ref flag5, ref flag6))
					{
						Color color2;
						if (flag6)
						{
