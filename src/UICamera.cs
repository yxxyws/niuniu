...
public class UICamera : MonoBehaviour
{
	...
	public static UICamera.ControlScheme currentScheme
	{
		...
		set
		{
			...
			{
				if (value == UICamera.ControlScheme.Controller)
				{
					UICamera.currentKey = KeyCode.JoystickButton0;
				}
				else
				{
					if (value == UICamera.ControlScheme.Touch)
					{
		...
		set
		{
			if (UICamera.mCurrentKey != value)
			{
				...
				if (controlScheme != UICamera.mLastScheme)
				{
					...
					{
						if (UICamera.current != null && UICamera.current.autoHideCursor)
						{
					...
					if (vector.x >= 0f && vector.x <= 1f && vector.y >= 0f && vector.y <= 1f)
					{
						...
						else
						{
							...
							else
							{
								...
								{
									if (uICamera.eventType == UICamera.EventType.UI_2D)
									{
										if (UICamera.m2DPlane.Raycast(ray, out num))
										{
	...
	private void ProcessEvents()
	{
		...
		if (this.useMouse && UICamera.mHover != null && UICamera.currentScheme == UICamera.ControlScheme.Mouse)
		{
	...
	public void ProcessOthers()
	{
		...
		else
		{
			...
			{
				if ((this.submitKey0 == KeyCode.Return || this.submitKey1 == KeyCode.Return) && UICamera.GetKeyDown(KeyCode.KeypadEnter))
				{
					UICamera.currentKey = this.submitKey0;
					flag = true;
				}
		...
		else
		{
			...
			{
				if ((this.submitKey0 == KeyCode.Return || this.submitKey1 == KeyCode.Return) && UICamera.GetKeyUp(KeyCode.KeypadEnter))
				{
					UICamera.currentKey = this.submitKey0;
					flag2 = true;
				}
	...
	private void ProcessPress(bool pressed, float click, float drag)
	{
		...
		{
			if (UICamera.currentTouch.pressed != null && (UICamera.currentTouch.delta.sqrMagnitude != 0f || UICamera.currentTouch.current != UICamera.currentTouch.last))
			{
				...
				{
					if (!UICamera.currentTouch.dragStarted && drag < sqrMagnitude)
					{
						flag = true;
						UICamera.currentTouch.dragStarted = true;
						UICamera.currentTouch.delta = UICamera.currentTouch.totalDelta;
				...
				if (UICamera.currentTouch.dragStarted)
				{
					...
					{
						if (UICamera.currentTouch.last != UICamera.currentTouch.current)
						{
	...
	public void ProcessTouch(bool pressed, bool released)
	{
		...
		if (UICamera.currentTouch.pressed != null)
		{
			...
			if (UICamera.currentTouch.deltaTime > this.tooltipDelay && UICamera.currentTouch.pressed == UICamera.currentTouch.current && UICamera.mTooltipTime != 0f && !UICamera.currentTouch.dragStarted)
			{
				UICamera.mTooltipTime = 0f;
				UICamera.currentTouch.clickNotification = UICamera.ClickNotification.None;
