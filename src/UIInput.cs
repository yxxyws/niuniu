...
public class UIInput : MonoBehaviour
{
	...
	public string Validate(string val)
	{
		...
		for (int i = 0; i < val.Length; i++)
		{
			...
			{
				if (this.validation != UIInput.Validation.None)
				{
	...
	protected char Validate(string text, int pos, char ch)
	{
		...
		else
		{
			...
			{
				if (this.validation == UIInput.Validation.Alphanumeric)
				{
					if (ch >= 'A' && ch <= 'Z')
					{
						return ch;
				...
				{
					if (this.validation == UIInput.Validation.Username)
					{
						if (ch >= 'A' && ch <= 'Z')
						{
							return ch - 'A' + 'a';
