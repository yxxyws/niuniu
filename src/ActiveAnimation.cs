...
public class ActiveAnimation : MonoBehaviour
{
	...
	private void Play(string clipName, Direction playDirection)
	{
		...
		if (this.mAnim != null)
		{
			...
			{
				if (!this.mAnim.IsPlaying(clipName))
				{
			...
			try
			{
				while (enumerator.MoveNext())
				{
					...
					if (string.IsNullOrEmpty(clipName) || animationState.name == clipName)
					{
						...
						{
							if (playDirection == Direction.Forward && animationState.time == animationState.length)
							{
								animationState.time = 0f;
							}
						}
	...
	public static ActiveAnimation Play(Animation anim, string clipName, Direction playDirection, EnableCondition enableBeforePlay, DisableCondition disableCondition)
	{
		...
		{
			if (activeAnimation.mAnimator != null)
			{
