...
public static class NGUIMath
{
	...
	public static Vector2 GetPivotOffset(UIWidget.Pivot pv)
	{
		...
		{
			if (pv == UIWidget.Pivot.TopRight || pv == UIWidget.Pivot.Right || pv == UIWidget.Pivot.BottomRight)
			{
				zero.x = 1f;
			}
			else
	...
	public static void AdjustWidget(UIWidget w, float left, float bottom, float right, float top, int minWidth, int minHeight, int maxWidth, int maxHeight)
	{
		...
		{
			if (pivotOffset.x == 1f && pivotOffset.y == 0f)
			{
				zero.x = vector4.x;
				zero.y = vector4.y;
			}
			...
			{
				if (pivotOffset.x == 0f && pivotOffset.y == 0f)
				{
					zero.x = vector3.x;
					zero.y = vector3.y;
				}
				...
				{
					if (pivotOffset.x == 1f && pivotOffset.y == 1f)
					{
						zero.x = vector2.x;
						zero.y = vector2.y;
					}
					...
					{
						if (pivotOffset.x == 0f && pivotOffset.y == 0.5f)
						{
						...
						{
							if (pivotOffset.x == 1f && pivotOffset.y == 0.5f)
							{
							...
							{
								if (pivotOffset.x == 0.5f && pivotOffset.y == 1f)
								{
								...
								{
									if (pivotOffset.x == 0.5f && pivotOffset.y == 0f)
									{
									...
									{
										if (pivotOffset.x == 0.5f && pivotOffset.y == 0.5f)
										{
