...
public abstract class UITweener : MonoBehaviour
{
	...
	private void Update()
	{
		...
		{
			if (this.style == UITweener.Style.PingPong)
			{
				if (this.mFactor > 1f)
				{
	...
	public void Sample(float factor, bool isFinished)
	{
		...
		{
			if (this.method == UITweener.Method.EaseOut)
			{
			...
			{
				if (this.method == UITweener.Method.EaseInOut)
				{
				...
				{
					if (this.method == UITweener.Method.BounceIn)
					{
						num = this.BounceLogic(num);
					}
					else
					{
						if (this.method == UITweener.Method.BounceOut)
						{
