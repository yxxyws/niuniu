...
public class UIWidget : UIRect
{
	...
	public void SetDimensions(int w, int h)
	{
		if (this.mWidth != w || this.mHeight != h)
		{
			...
			else
			{
				...
				{
					if (this.keepAspectRatio == UIWidget.AspectRatioSource.Free)
					{
	...
	public bool UpdateTransform(int frame)
	{
		...
		{
			if (!this.panel.widgetsAreStatic && cachedTransform.hasChanged)
			{
				this.mMatrixFrame = -1;
				cachedTransform.hasChanged = false;
	...
	public bool UpdateGeometry(int frame)
	{
		...
		{
			if (this.mMoved && this.geometry.hasVertices)
			{
				if (this.mMatrixFrame != frame)
				{
					this.mLocalToPanel = this.panel.worldToLocal * base.cachedTransform.localToWorldMatrix;
